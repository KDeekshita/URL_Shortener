...
function App() {
  ...
  const handleShorten = async () => {
    ...
    try {
      const res = await axios.post(`${API_BASE_URL}/api/url/shorten`, {
        originalUrl: url,
      });
      ...
      // Only alert if backend really failed
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6">
      ...
      <div className="flex flex-col gap-3 w-full max-w-3xl">
        <input
          ...
          placeholder={shortUrl ? "Enter new URL" : "Enter long URL"}
          value={url}
      ...
      {shortUrl && (
        <div className="flex flex-col items-center max-w-3xl w-full">
          ...
          <a
            className="link link-primary break-all" target="_blank"
            href={shortUrl} rel="noreferrer"
          >
            {shortUrl}
          </a>
          ...
          <p className="mt-4 text-sm text-gray-400">
            Enter another URL above to shorten again
          </p>
        </div>
      )}
    </div>
